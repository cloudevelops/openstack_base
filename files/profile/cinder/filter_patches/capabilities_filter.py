...
class CapabilitiesFilter(filters.BaseHostFilter):
    ...
    def _satisfies_extra_specs(self, capabilities, resource_type):
        ...
        for key, req in six.iteritems(extra_specs):
            ...
            if not extra_specs_ops.match(cap, req):
                #if not (cap == "ceph_hdd" and req == "lvm_ssd") or not (cap == "lvm_ssd" and req == "ceph_hdd"):
    ...
    def host_passes(self, host_state, filter_properties):
        ...
        if not self._satisfies_extra_specs(host_state.capabilities,
                                           resource_type):
            if ((host_state.capabilities['volume_backend_name'] == 'lvm_ssd') and (resource_type['extra_specs']['volume_backend_name'] == 'ceph_hdd')):
                LOG.debug("CLOUDINFRASTACK: Override CapabilitiesFilster: Host_state.host: %(host)s, Host_state.capabilities.volume_backend_name: %(capabilities)s, Resource_type.extra_specs.volume_backend_name: %(resource_type)s ",
                      {'host': host_state.host, 'capabilities': host_state.capabilities['volume_backend_name'], 'resource_type': resource_type['extra_specs']['volume_backend_name']})
                return True
